...
from utils import DateUtils, AsyncHelper, Logger
...
def main():
    ...
    # --- Tiêu đề ---
    st.html("""<h1 class="main-header">👨‍👩‍👧‍👦 <i>Trợ lý Gia đình</i> 💬</h1>""")
    ...
    with st.sidebar:
        ...
        # Hàm reset conversation
        def reset_conversation():
            if "messages" in st.session_state and len(st.session_state.messages) > 0:
                # Trước khi xóa, lưu lịch sử trò chuyện nếu đang trò chuyện với một thành viên
                if st.session_state.current_member and st.session_state.openai_service:
    ...
    if not openai_api_key or "sk-" not in openai_api_key:
        ...
        # Hàm xử lý lệnh từ phản hồi của AI
        def process_commands(response: str, current_member: Optional[str] = None):
            ...
            if "ADD_EVENT" in commands:
                ...
                st.session_state.db_manager.add_event(details)
            ...
            if "UPDATE_PREFERENCE" in commands:
                ...
                st.session_state.db_manager.update_preference(member_id, key, value)
            ...
            if "ADD_NOTE" in commands:
                ...
                st.session_state.db_manager.add_note(details)
                st.success("Đã thêm ghi chú!")
        
        # Hàm để xử lý phản hồi từ trợ lý AI
        ...
        # Hàm xử lý khi người dùng chọn câu hỏi gợi ý
        ...
        if st.session_state.openai_service:
            ...
            if "question_cache" not in st.session_state or cache_key not in st.session_state.question_cache:
                ...
                for event_id, event in events_data.items():
                    try:
                        ...
                        if event_date >= today:
                            ...
                            if date_diff <= 14:  # Chỉ sự kiện trong 2 tuần tới
                ...
                suggested_questions = st.session_state.question_cache[cache_key]
